...
namespace LyricalMiracle.BusinessLogic
{
    public class LoginLogic
    {
        ...
        LoginData loginDataInstance = new LoginData();

        public bool VerifyPassword(string username, string passwordToCompare)
        {
            string passwordDB = string.Empty;

            try
            {
                passwordDB = loginDataInstance.GetHashedPassword(username);

                if (VerifyHash(passwordToCompare, passwordDB))
                {
                    return true;
                }
            ...
            {
                exceptionInstance.WriteException(Guid.NewGuid(), ex, "LoginLogic", "VerifyPassword", DateTime.Now);
            }
        ...
        public string ComputeHash(string password)
        {
            ...
            try
            {
                hashedPassword = ComputeHash(password, null);
        ...
        public string ReturnHash(string username, string passwordToHash)
        {
            string hashedPassword = string.Empty;

            try
            {
                hashedPassword = ComputeHash(passwordToHash, null);

            }
            catch (Exception ex)
            {
                exceptionInstance.WriteException(Guid.NewGuid(), ex, "LoginLogic", "ReturnHash", DateTime.Now);
            }

            return hashedPassword;
        }

        #region Hashsing + Salt
        /// <summary>
